...
Landmarks (strict order g < n < prn < cm < sn < ls < pg):
  ...
  cm  (valley) — always use the "det" value exactly
  sn  (valley) — base of nose, pick lowest P_value from candidates
  ls  (peak)   — upper lip, pick highest P_value from candidates
...
- cm: always return the "det" value
...
const LM_YN_CENTER = { g: 0.190, n: 0.285, prn: 0.465, sn: 0.600, ls: 0.740, li: 0.790, pg: 0.890 };
const LM_YN_HALF   = { g: 0.100, n: 0.120, prn: 0.115, sn: 0.080, ls: 0.070, li: 0.045, pg: 0.110 };
...
const GAP_REFS = [
  ...
  [2, 4, 0.100, 0.030, 0.60], // prn→sn: tight cluster in nose area
  [4, 5, 0.140, 0.050, 0.50], // sn→ls  (most critical, was the main bug)
...
function inferColumellaIndex(prnIdx, snIdx, smoothed, kappa, promValley) {
  ...
  const maxAllowed = prnIdx + Math.round(span * 0.72); // do not drift too close to sn
...
function collectAllCandidates(smoothed, Pd1, kappa, yNorm, promPeak, promValley, faceH, yn2i, globalMax, N) {
  ...
  const zones = {
    ...
    sn:  [yn2i(0.52), yn2i(0.61)],   // tighter end — prn/cm/sn must cluster in nose area
    ls:  [yn2i(0.68), yn2i(0.84)],   // start after sn zone ends
...
function inferDeterministicFromContour(contour, width, side) {
    ...
    if (!dpResult) {
      ...
      if (pgIdx > pgHardMax) {
        ...
        if (pgReZE > pgReZS) {
          ...
          pgIdx = pgReR.idx; pgScore = pgReR.score; pgMargin = pgReR.margin ?? 0; pgFallback = pgReR.method === 'fallback';
          console.log(`[ProfileContour] DP pg re-capped: ${dpPgIdx}→${pgIdx} (lsIdx=${lsIdx} hardMax=${pgHardMax})`);
        }
      }
      ...
      scrArr    = [dpResult.scrArr[0] * 0.70, nArgScore, dpResult.scrArr[2], cmScore,
                   dpResult.scrArr[3], dpResult.scrArr[4], pgScore];
      marginArr = [dpResult.marginArr[0], nArgMargin, dpResult.marginArr[2], 0,
                   dpResult.marginArr[3], dpResult.marginArr[4], pgMargin];
      fallbackArr = [
        dpResult.fallbackArr[0], false, dpResult.fallbackArr[2],
        cmR.source === 'fallback',
        dpResult.fallbackArr[3], dpResult.fallbackArr[4], pgFallback,
      ];
      usedDP = true;
    }
  } catch (e) {
  ...
  if (!usedDP) {
    ...
    // Step 4: subnasale — y_norm [0.52..0.64], hard cap at prnIdx + 15% faceH.
    // prn/cm/sn are all nose-area landmarks and must cluster tightly on the face.
    const snHardMax = Math.min(N - 1, prnIdx + Math.round(faceH * 0.15));
    const snZS = Math.max(prnIdx + 2, yn2i(0.52)), snZE = Math.min(snHardMax, yn2i(0.64));
    ...
    // Step 6: ls — constrained by sn-relative gap to avoid drifting toward subnasale
    ...
    if (lsZE <= lsZS) {
      lsZS = Math.max(snIdx + Math.max(2, Math.round(faceH * 0.08)), yn2i(0.65));
      lsZE = yn2i(0.80);
    }
    const lsR = bestInZone(smoothed, Pd1, kappa, yNorm, promPeak, promValley, lsZS, lsZE, 'ls');
    const lsIdx = lsR.idx;

    // Step 7: pg — y_norm [0.78..1.00], hard cap at ls + PG_MAX_GAP_RATIO*faceH.
    // This prevents pg from drifting into neck when the contour continues past chin.
    const pgHardMax = Math.min(N - 1, lsIdx + Math.round(faceH * PG_MAX_GAP_RATIO));
    const pgZS = Math.max(lsIdx + 2, yn2i(0.78)), pgZE = Math.min(pgHardMax, yn2i(1.00));
  ...
  // Step 8: li (labrale inferius) — optional valley between ls and pg
  const liMinGap = Math.max(2, Math.round(faceH * 0.03));
  const liZS = idxArr[5] + liMinGap;
  const liZE = Math.max(liZS + 1, idxArr[6] - liMinGap);
  const liR = liZE > liZS + 2
    ? bestInZone(smoothed, Pd1, kappa, yNorm, promPeak, promValley, liZS, liZE, 'li')
    : null;
  const liIdx = liR ? liR.idx : null;

  console.log(`[ProfileContour] path=${usedDP ? 'DP' : 'seq'} cm=${cmR.source}:${cmR.candidateCount} li=${liR ? liR.method : 'skip'}`);
  ...
  const indices = { g: idxArr[0], n: idxArr[1], prn: idxArr[2], cm: idxArr[3], sn: idxArr[4], ls: idxArr[5], pg: idxArr[6] };
  if (liIdx !== null) indices.li = liIdx;
...
function validateIndices(indices, contour, width, height, side) {
  ...
  if (uniq.size < 7) return { ok: false, reason: 'duplicate_indices' };
...
function resolveResultForProfile(profile, aiBySide, detectorBySide) {
  ...
  const valid = validateIndices(ai.indices, profile.contour, profile.imageWidth, profile.imageHeight, profile.side);
  if (!valid.ok) {
  ...
  if (ai.overallConfidence < AI_MIN_CONFIDENCE) {
    if (detector) return { source: 'detector', reason: 'ai_low_confidence', ...detector };
    return { source: 'detector', reason: 'ai_low_confidence' };
  }
...
function buildCandidateWindows(detectorResult, smoothed, N) {
  ...
  for (const key of REQUIRED_KEYS) {
    ...
    const hi = Math.min(N - 1, center + WINDOW);
    const candidates = [];
    for (let i = lo; i <= hi; i++) {
...
export async function handleProfileLandmarks(req, res) {
  ...
  // Phase 1: deterministic-only, AI disabled
  const model = resolveProfileModel();
  const aiBySide = new Map();
  const aiFailureReason = 'disabled_phase1';
